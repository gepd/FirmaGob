...
export class FirmaGob {
  ...
  private run = "11111111";
  private purpose = Purpose.ATENDIDO;
   ...
   * @param xmlObjects array de string con los pie de firma en un archivo XML
                       ejemplo: ["<a></a>","<b/>”]
   */
  addXML(content: string, checksum: string, references: string[], xmlObjects: string[]) {
    this.files.push({
  ...
  signFiles(otp?: string) {
    ...
    expiration.setTime(expiration.getTime() + THIRTY_MINUTES); // Agrega 30 minutos
    ...
    const payload_enc = Buffer.from(payload_str).toString("base64").replace(/\=/g, "");

    const unsigned_token = `${header_enc}.${payload_enc}`;

    const signature_str = createHmac("sha256", this.secret).update(unsigned_token);
    const signature_enc = signature_str.digest("base64").replace(/\=/g, "");

    const token = `${unsigned_token}.${signature_enc}`;

    const headers = otp && this.purpose === Purpose.ATENDIDO ? { otp } : undefined;
    ...
    return fetch(this.url, { method: "post", body: data, headers });
  }
}
